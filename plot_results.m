...
out_file = "Test";
DEVICES = ["ICS02_Final"];%, "SPU02_2_DPKG_MEMFRONT"];%, "ADMP01_DPKG_MEMFRONT"];
CUSTOM_FILES = [];
...
OUT_COEFFICIENTS = [ones(1, NUM_FREQS); 2.0444*ones(1, NUM_FREQS)]; % To account for Vpp vs mV Amplitude
LIGHT_FREQUENCIES = ["1470nm"];
DC_POWERS = ["5mW"];
AC_POWERS = ["2mWpp"];
PRESSURES = ["1atm" "0.1atm"];

NUM_FREQS = 100;
COMBINE_OUTPUTS = ["ICS02_Final"];% ["SPU02_DPKG_MEMFRONT", "SPU02_DPKG_ASICTOP_450nm_5mW_0.33mWpp_1atm"];%["VM02_DPKG_MEMFRONT","VM02_DPKG_ASICTOP"];
            ...
            for ac_power = AC_POWERS
                for light_freq = LIGHT_FREQUENCIES
                    ...
                    if ~isempty(COMBINE_OUTPUTS) && ismember(dev, COMBINE_OUTPUTS)
                        complex_vals = COMBINE_COEFFICIENTS(k,:).*amp_outs(i,:).*exp(1j*deg2rad(phase_outs(i,:)));
